	...
	public function executeIndex(\Library\HTTPRequest $request)
	{
		...
		if ($request->postExists('modifier')) {
			if ($request->postExists('check')) {
				...
				if (count($check) > 1) {
					$this->app->user()->setFlash('warning','<strong>Attention !</strong> Vous ne pouvez modifier qu\'un élève à la fois');
				} else {
					$this->app->httpResponse()->redirect('/admin/eleves/modifier-'.$check[0]);
				}
			} else {
				$this->app->user()->setFlash('warning','<strong>Attention !</strong> Vous devez sélectionner au moins un élève pour le modifier');
			}
		...
		if($request->postExists('ajouter')) {
			...
			if($eleve->isValid()) {
				...
				if($record instanceof \Library\Entities\Eleve)
				{
					...
					if($envoi == 1)
					{
						$this->app->user()->setFlash('success','Enregistrement de l\'élève réussi');
					}
					else
					{
	...
	public function executeModif(\Library\HTTPRequest $request)
	{
		...
		if($eleve != NULL) {
			...
			if($request->postExists('modifier')) {
				...
				if($el->isValid()) {
					$record = $this->managers->getManagerOf('Eleve')->save($el);					
