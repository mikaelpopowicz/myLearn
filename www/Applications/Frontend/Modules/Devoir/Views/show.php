				...
				<p>
					...
					if($devoir->pieces() && is_array($devoir->pieces()))
					{
						...
						foreach ($devoir->pieces() as $fichier) {
							echo "<li>";
							echo "<a target='blank' href='/devoirs/consulter-".$devoir->id()."/piece-".$fichier->id()."'>".$fichier->libelle()."</a>";
							echo "</li>";
						}
						echo "</ul>";
					}
							...
							<tbody id="tabs">
								...
								if(is_array($devoir->rendus()->pieces()) && !empty($devoir->rendus()->pieces()))
								{
									foreach ($devoir->rendus()->pieces() as $piece) {
										...
										echo "\n\t\t\t\t\t\t\t</td>";
										echo "\n\t\t\t\t\t\t\t<td>\n\t\t\t\t\t\t\t\t<a target='blank' href='/devoirs/consulter-".$devoir->id()."/production-".$piece->id()."'>".$piece->libelle()."</a>\n\t\t\t\t\t\t\t</td>";
